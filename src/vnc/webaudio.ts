...
export default class WebAudio {
    ...
    private queue: ArrayBuffer | null = null;
    private lastPacket: number | null = null;
    private connected: boolean = false;
    private maximumAudioLag: number;
    private syncLagInterval: number
    private updateBufferEvery: number;
    private reduceBufferInterval: number
    private maximumSecondsOfBuffering: number;
    ...
    onSourceOpen() {
        this.buffer = this.mediaSource?.addSourceBuffer('audio/mp4; codecs="mp4a.40.2"') ?? null;
        this.registerHandlers();
        this.wsConnect();
    }

    //whenever data arrives in our websocket this is called.
    websocketDataArrived(e: MessageEvent) {
